...
// Get all approved UMKM (public) - NO submission photos included
...
// Get all pending UMKM with their submission photos (admin only)
...
// Submit new UMKM (public) - photo stored separately
router.post("/submit", async (req, res) => {
  try {
    const { nama_umkm, kategori_usaha, alamat, nomor_telepon, email, foto_url } = req.body
    ...
    const [result] = await db.execute(
      `INSERT INTO umkm (nama_umkm, kategori_usaha, alamat, nomor_telepon, email, status)
       VALUES (?, ?, ?, ?, ?, 'pending')`,
      [nama_umkm, kategori_usaha, alamat, nomor_telepon, email || null],
    )
...
router.post("/:id/approve", verifyToken, async (req, res) => {
  try {
    ...
    const [result] = await db.execute(
      `UPDATE umkm SET status = "approved", approved_by = ?, approved_at = NOW()
       WHERE id = ?`,
      [req.admin_id, id],
    )
