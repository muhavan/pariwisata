...
// ===== LOAD UMKM DATA =====
async function loadUMKMTable(limit = null) {
  const tableBody = document.getElementById("umkm-table-body")
  if (!tableBody) return

  try {
    let url = "http://localhost:5000/api/umkm"
    if (limit) {
      url += `?limit=${limit}`
    }
...
// ===== LOAD FEATURED DESTINASI (max 3) =====
async function loadFeaturedDestinasi() {
  const container = document.getElementById("destinasi-home")
  if (!container) return

  try {
    const response = await fetch("http://localhost:5000/api/wisata?limit=3")
    const data = await response.json()

    container.innerHTML = ""

    if (!data.success || !data.data || data.data.length === 0) {
      container.innerHTML = '<p style="text-align: center; color: #9ca3af;">Belum ada destinasi wisata</p>'
      return
    }
    ...
    data.data.forEach((item) => {
      ...
      card.innerHTML = `
        <div class="destinasi-image">
          <img src="${item.foto_url || "/destinasi.jpg"}" alt="${item.nama_destinasi}">
        </div>
    ...
    container.innerHTML = '<p style="text-align: center; color: #d32f2f;">Gagal memuat destinasi</p>'
  }
}

// ===== LOAD FEATURED BERITA (max 3) =====
async function loadFeaturedBerita() {
  const container = document.getElementById("berita-home")
  if (!container) return

  try {
    const response = await fetch("http://localhost:5000/api/berita?limit=3")
    const data = await response.json()

    container.innerHTML = ""

    if (!data.success || !data.data || data.data.length === 0) {
      container.innerHTML = '<p style="text-align: center; color: #9ca3af;">Belum ada berita</p>'
      return
    }
    ...
    data.data.forEach((item) => {
      ...
      card.innerHTML = `
        <div class="berita-image">
          <img src="${item.foto_url || "/berita.jpg"}" alt="${item.judul}">
        </div>
    ...
    container.innerHTML = '<p style="text-align: center; color: #d32f2f;">Gagal memuat berita</p>'
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Load featured items on homepage (5 UMKM, 3 destinasi, 3 berita)
  loadUMKMTable(5)
  loadFeaturedDestinasi()
  loadFeaturedBerita()
})
